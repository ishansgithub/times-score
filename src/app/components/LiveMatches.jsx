  ...
  const getData = async () => {
    ...
    try {
      const response = await fetch(
        "https://api.cricapi.com/v1/currentMatches?apikey=10812d1c-bf92-4d8d-8f06-eeae16c1e80d&offset=0"
      );
    ...
    <div className="min-h-screen bg-black">
      <div className="container mx-auto px-4">
        <div className="pt-12 pb-8 text-center">
          <h1 className="text-4xl font-light text-white tracking-wide mb-2 font-serif">LIVE MATCHES</h1>
          ...
          {isLoading ? (
            ...
            data.map((match) => {
              if (
                ...
                match.teams?.some(team => team.toLowerCase().includes(search.toLowerCase()))
              ) {
                return (
                  <div 
                    key={match.id} 
                  ...
                  >
                    <div className="p-4 border-b border-white/10">
                      <h3 className="text-sm font-light text-white/90 tracking-wide">{match.name}</h3>
                      <div className="flex justify-between items-center mt-1">
                        <p className="text-xs text-white/60 font-normal">{match.matchType}</p>
                        <p className="text-xs text-white/60 font-normal">{new Date(match.dateTimeGMT).toLocaleDateString()}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-6">
                        ...
                        {match.teams.map((team, idx) => (
                          <div key={team} className="flex justify-between items-center">
                            <div className="flex-1">
                              <p className="font-light text-white/80 text-sm mb-1">{team}</p>
                              {match.score && match.score[idx] && (
                                <p className="text-lg text-white font-light">
                                  {formatScore(match.score[idx])}
                                </p>
                              )}
                            </div>
                          </div>
                      ...
                      <div className="mt-6 pt-4 border-t border-white/10">
                        <p className="text-sm text-white/70 font-light text-center mb-2">
                          {match.status}
                        </p>
